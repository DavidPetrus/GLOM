...
class JHMDB_Dataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        for i in range(self.batch_size):
            f_ix = np.random.randint(0,18)
            count = 0
            # Select sample
            video_file = self.video_files[self.batch_size*index + i]
            ...
            while True:
                ...
                if count == f_ix:
                    frame = frame[:frame.shape[0]-frame.shape[0]%self.granularity[0],:frame.shape[1]-frame.shape[1]%self.granularity[0],::-1]
                    img = torch.from_numpy(np.ascontiguousarray(frame)).float()
                    img = img.movedim(2,0)
...
class ADE20k_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_size):
            ...
            img = img[:img.shape[0]-img.shape[0]%self.granularity[0],:img.shape[1]-img.shape[1]%self.granularity[0]]
